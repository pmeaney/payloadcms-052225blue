...
# Database configuration (matching your actual production environment)
POSTGRES_DB="payloadcms-db-052225blue"
POSTGRES_USER="payloadcms-052225blue-user"

# Default values (corrected for your actual environment)
LOCAL_DB_CONTAINER="payloadcms-db-052225blue"
PROD_DB_CONTAINER="payloadcms-db-052225blue"  # From your CI/CD files - this is the PRODUCTION DB container
...
# 5. Create backup of production database (complete)
...
# Process the complete backup file
DATA_MODIFIED="$LOCAL_BACKUP_DIR/modified_$DATA_FILENAME"
log "Processing complete backup file..."

# For complete dumps, we can be less aggressive with filtering since it includes schema
cat "$LOCAL_BACKUP_DIR/$DATA_FILENAME" | \
  # Remove any problematic owner/privilege statements
  grep -v "^ALTER.*OWNER TO" | \
  grep -v "^GRANT\|^REVOKE" > "$DATA_MODIFIED"

# Method: Restore complete database (schema + data)
log "\n${YELLOW}Restoring complete production database...${NC}"
